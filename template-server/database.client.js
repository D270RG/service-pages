...
async function addPrice(priceObject) {
  ...
  return {
    affected:
      Helper.checkAffected(affectedDescriptions) &
      Helper.checkAffected(affectedPrices),
  };
}
async function addFlyer(flyerObject) {
  ...
  return {
    affected:
      Helper.checkAffected(affectedFlyers) &
      Helper.checkAffected(affectedDescriptions),
    id: uuid,
  };
}
...
async function deleteFlyer(id) {
  const affectedFlyers = await Connector.set(
    `DELETE FROM Flyers WHERE id=${id}`
  );
  const affectedDescriptions = await Connector.set(
