...
function AppTabsContent() {
	...
	useEffect(() => {
		...
		fetch(`http://${serverAddress}/prices`, {
			...
			.then((priceData) => {
				console.log('requested', priceData);
				// const examplePriceRequest = {['first-help','laptop-repair','phone-repair']}; //POST with all TabMap keys
							...
							<Route
								...
								element={
									<React.Suspense
										...
										{priceData.hasOwnProperty(pathKey) && (
											<PriceTable
												priceInfo={priceData}
												path={pathKey}
											/>
